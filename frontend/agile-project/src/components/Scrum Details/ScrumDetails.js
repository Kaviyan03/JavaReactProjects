            ...
            try
             {
                ...
                const assignedUserIds = tasksResponse.data.map(task => String(task.assignedTo));
                const scrumUsers = usersResponse.data.filter(user => assignedUserIds.includes(user.id));
